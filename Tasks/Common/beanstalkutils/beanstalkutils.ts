...
"aspNetCoreWeb": [
  {
    ...
    public static async uploadBundle(s3Client: S3,
        ...
        try {
            const response: S3.ManagedUpload.SendData = await s3Client.upload({
                Bucket: bucketName,
                Key: objectKey,
                Body: fileBuffer
            }).promise();
    ...
    public static async verifyEnvironmentExists(beanstalkClient: Beanstalk,
                                                applicationName: string,
                                                environmentName: string): Promise<void> {
