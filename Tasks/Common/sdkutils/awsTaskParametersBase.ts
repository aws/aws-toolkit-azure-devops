...
import {
    ...
    awsSessionTokenVariable
} from '../awsConnectionParameters'
...
export class AWSTaskParametersBase implements AWSConnectionParameters {
    ...
    public constructor() {
        ...
        this.proxyConfiguration = tl.getHttpProxyConfiguration() || process.env.HTTPS_PROXY || process.env.HTTP_PROXY
    ...
    private attemptEndpointCredentialConfiguration(endpointName: string): AWS.Credentials {
        ...
        if (customDurationVariable) {
            ...
            if (isNaN(customDuration) || customDuration < minDuration || customDuration > maxduration) {
                console.warn(
                    `Invalid credential duration '${customDurationVariable}', minimum is ${
                        this.minDuration
                    } seconds, max ${this.maxduration} seconds`
                )
            } else {
    ...
    private attemptCredentialConfigurationFromVariables(): AWS.Credentials {
        ...
        if (!secretKey) {
            throw new Error(
                'AWS access key ID present in task variables but secret key value is missing; cannot configure task credentials.'
            )
        }
    ...
    public async getRegion(): Promise<string> {
        ...
        if (region) {
            console.log(
                `...configured to use region ${region}, defined in task variable ${
                    AWSTaskParametersBase.awsRegionVariable
                }.`
            )
            return region
        }
