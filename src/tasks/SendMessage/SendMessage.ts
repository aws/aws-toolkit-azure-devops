...
import { createDefaultSNS, createDefaultSQS } from 'lib/defaultClients'
...
async function run(): Promise<void> {
    ...
    return new TaskOperations(
        await createDefaultSNS(taskParameters, tl.debug),
        await createDefaultSQS(taskParameters, tl.debug),
        taskParameters
    ).execute()
}
