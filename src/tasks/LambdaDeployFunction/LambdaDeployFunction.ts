...
import { createDefaultIAM, createDefaultLambda } from 'lib/defaultClients'
...
async function run(): Promise<void> {
    ...
    return new TaskOperations(
        await createDefaultIAM(taskParameters, tl.debug),
        await createDefaultLambda(taskParameters, tl.debug),
        taskParameters
    ).execute()
}
